# [FUNCTION]: 提供 UI 元素的物理几何计算，确保窗口绝对对齐与底座比例锁定。
...
def get_bottom_center_pos(screen_w, screen_h, win_w, win_h, offset_y=60):
    ...
    offset_y: 距离屏幕最底部的距离，建议 60 像素以避开任务栏。
    """
    x = (screen_w - win_w) // 2
    y = screen_h - win_h - offset_y
    return x, y

def get_centering_pos(parent_size: int, child_size: int) -> int:
    """计算绝对中心偏移量，用于 Canvas 内部元素的对齐"""
